...
def evaluate_mcts_vs_mcts(
    ...
    """Avalia dois agentes MCTS um contra o outro (X vs O).

    Args:
        env:        Ambiente Tic-Tac-Toe (reutilizado; é reposto em cada jogo).
        agent_x:    MCTS que joga como X (+1); pode ser o mesmo objeto que ``agent_o``
                    se tiverem a mesma configuração (o agente é sem estado entre jogadas).
        agent_o:    MCTS que joga como O (−1).
        n_games:    Número de jogos a simular.

    Returns:
        ``(x_win_rate, draw_rate, o_win_rate)`` — três frações que somam 1.
    """
    ...
    for _ in range(n_games):
        ...
        while not done:
            if env.current_player == 1:
                action = agent_x.select_action(state, 1)
            else:
                action = agent_o.select_action(state, -1)
            state, _, done = env.step(action)
        winner = _winner(state)
        if winner == 1:
            x_wins += 1
            ...
            o_wins += 1
    n = float(n_games)
    return x_wins / n, draws / n, o_wins / n
